...
def generate_launch_description():
    ...
    # Wheel odometry: publishes /odom (message only), no TF
    wheel_odometry = Node(
        package='smart_car',
        ...
        parameters=[
            {'use_sim_time': True},
            {'broadcast_tf': False}  # IMPORTANT: EKF will own the TF
        ],
        emulate_tty=True,
    ...
    # EKF: fuses /odom + /imu_data and publishes the TF (odom -> base_footprint)
    ekf_node = Node(
        ...
        # Do NOT remap odometry/filtered -> /odom; EKF should publish TF, not override /odom topic
    )
